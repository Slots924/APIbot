"""Допоміжний модуль із логікою сортування коментарів за найновішими."""
...
def sort_comments_by_newest(driver) -> bool:
    """Відкриває меню сортування Facebook і обирає пункт «Найновіші».

    Функція винесена в окремий файл, щоб її можна було легко використовувати
    як у дії `like_comments`, так і в інших сценаріях. Вона складається з
    трьох етапів: пошуку кнопки меню, відкриття цього меню та вибору потрібної
    опції. Кожен крок супроводжується детальними логами українською мовою, щоб
    легше відслідковувати помилки під час налагодження.
    """
    ...
    keywords = [
        "most recent", "newest",         # EN
        "найнов", "нові",                # UA
        "самые новые", "новые"           # RU
    ]
